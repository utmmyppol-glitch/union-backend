set -eu

NETWORK="union-net"
ALIAS="union-backend-active"
IMAGE="${DOCKER_IMAGE:?DOCKER_IMAGE not set}"

# 네트워크 없으면 최초 1회 생성 (db도 이 네트워크에 붙어있어야 함)
docker network create "$NETWORK" >/dev/null 2>&1 || true

if docker ps --format '{{.Names}}' | grep -qx union-backend-blue; then
  CURRENT=union-backend-blue
  NEW=union-backend-green
else
  CURRENT=union-backend-green
  NEW=union-backend-blue
fi

echo "==> Using locally built image $IMAGE"
docker image inspect "$IMAGE" >/dev/null 2>&1 || {
  echo "ERROR: 이미지 $IMAGE 가 로컬에 없습니다. docker stage가 먼저 빌드했는지 확인하세요."
  exit 1
}

docker rm -f "$NEW" >/dev/null 2>&1 || true

echo "==> Starting $NEW on $NETWORK (not yet receiving traffic via $ALIAS)"
docker run -d --name "$NEW" --network "$NETWORK" --restart unless-stopped \
  -e SPRING_PROFILES_ACTIVE="${SPRING_PROFILES_ACTIVE:-prod}" \
  -e DB_URL="${DB_URL:-jdbc:postgresql://db:5432/union_integrated}" \
  -e DB_USERNAME="${DB_USERNAME:-postgres}" \
  -e DB_PASSWORD="${DB_PASSWORD:?DB_PASSWORD not set}" \
  -e SERVER_PORT="8081" \
  -e MAIL_HOST="${MAIL_HOST:-smtp.gmail.com}" \
  -e MAIL_PORT="${MAIL_PORT:-587}" \
  -e MAIL_USERNAME="${MAIL_USERNAME:-}" \
  -e MAIL_PASSWORD="${MAIL_PASSWORD:-}" \
  -e JWT_SECRET="${JWT_SECRET:?JWT_SECRET not set}" \
  -e CORS_ALLOWED_ORIGINS="${CORS_ALLOWED_ORIGINS:-http://localhost:3000,http://localhost:3001,http://localhost:3002}" \
  -e NAVER_APIGW_KEY_ID="${NAVER_APIGW_KEY_ID:-}" \
  -e NAVER_APIGW_KEY="${NAVER_APIGW_KEY:-}" \
  "$IMAGE"

echo "==> Health checking $NEW"
OK=0
i=0
while [ "$i" -lt 15 ]; do
  if docker run --rm --network "$NETWORK" curlimages/curl:8.10.1 -sf "http://$NEW:8081/actuator/health" >/dev/null 2>&1; then
    OK=1
    break
  fi
  i=$((i + 1))
  sleep 3
done

if [ "$OK" -ne 1 ]; then
  echo "==> Health check failed for $NEW, printing logs for debugging:"
  docker logs "$NEW" --tail 80 2>&1 || true
  echo "==> Rolling back (old container untouched)"
  docker rm -f "$NEW" >/dev/null 2>&1 || true
  exit 1
fi

echo "==> Switching traffic: attaching $NEW alias $ALIAS"
docker network disconnect "$NETWORK" "$NEW"
docker network connect --alias "$ALIAS" --alias "$NEW" "$NETWORK" "$NEW"

sleep 2

if docker ps -a --format '{{.Names}}' | grep -qx "$CURRENT"; then
  echo "==> Removing old container $CURRENT"
  docker network disconnect "$NETWORK" "$CURRENT" >/dev/null 2>&1 || true
  docker rm -f "$CURRENT" >/dev/null 2>&1 || true
fi

echo "==> Deploy complete. Active: $NEW"
